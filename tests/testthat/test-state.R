...
test_that("dragmapr_state records crs, geometry_id and selected_feature", {
  ...
  expect_error(d_state(geometry_id = c("a", "b")), "geometry_id")
  expect_error(d_state(crs = "not-a-crs"))
})
...
test_that("dragmapr_state_diff reports composition changes with tolerance", {
  ...
  diff <- d_state_diff(draft, canonical, tolerance = 1)
  ...
  expect_true(d_state_equal(canonical, canonical))
  expect_false(d_state_equal(draft, canonical, tolerance = 1))
})

test_that("dragmapr_state_diff separates composition from interaction", {
  canonical <- d_state(selected_feature = "A", view = list(scale = 1))
  draft <- d_state(selected_feature = "B", view = list(scale = 2))

  expect_true(d_state_equal(draft, canonical, compare = "composition"))
  expect_false(d_state_equal(draft, canonical, compare = "interaction"))
  expect_false(d_state_equal(draft, canonical, compare = "all"))
})
